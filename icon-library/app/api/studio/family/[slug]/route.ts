...
import { isStudioEnabled, readSourceFamily, writeSourceFamily, deleteSourceFamily } from '@/lib/studio';
...
export async function DELETE(
  ...
  const { slug } = await params;
  const ok = deleteSourceFamily(slug);
  if (!ok) return NextResponse.json({ error: 'Family not found' }, { status: 404 });
