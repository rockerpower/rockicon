...
export function BrowseShell({
  ...
  const login = useCallback(async (email: string) => {
    try {
      ...
      const data = await res.json();
      if (res.ok) { setSession(data.session); setLoginOpen(false); showToast(`Signed in as ${data.session.email}`); if (data.session.tier === 'pro') loadProVectors(); }
      else showToast(data.error ?? 'Login failed');
    } catch { showToast('Login failed'); }
  }, [showToast, loadProVectors]);

  const logout = useCallback(async () => {
    await fetch('/api/session', { method: 'DELETE' });
    setSession(null); setProVectors({}); showToast('Signed out');
  }, [showToast]);
    ...
    <div style={{ display: 'flex', height: '100vh', background: 'var(--background)', color: 'var(--foreground)', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <header style={{ flexShrink: 0, height: 56, display: 'flex', alignItems: 'center', gap: 12, padding: '0 20px', borderBottom: '1px solid var(--border)' }}>
          ...
          {session ? (
            ...
            <button onClick={() => setLoginOpen(true)} style={{ height: 32, padding: '0 14px', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--foreground)', fontSize: 12.5, fontWeight: 600, cursor: 'pointer' }}>Sign in</button>
          )}
        </header>
      ...
      {loginOpen && (
        <div
          ...
          style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,.5)' }}
        >
          <div
            onClick={e => e.stopPropagation()}
            style={{ width: 340, padding: 24, background: 'var(--background)', border: '1px solid var(--border)', borderRadius: 14, boxShadow: '0 20px 60px rgba(0,0,0,.5)' }}
          >
            <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 6 }}>Sign in</div>
            <p style={{ margin: '0 0 16px', fontSize: 12.5, color: 'var(--muted)', lineHeight: 1.5 }}>Enter your email to sign in. Required to unlock Pro icons.</p>
            <form onSubmit={e => { e.preventDefault(); if (loginEmail.trim()) login(loginEmail.trim()); }}>
              <input
                autoFocus
                type="email"
                value={loginEmail}
                ...
                style={{ width: '100%', height: 38, padding: '0 12px', background: 'var(--field)', border: '1px solid var(--border)', borderRadius: 9, color: 'var(--foreground)', fontSize: 13.5, outline: 'none', marginBottom: 12 }}
              />
              <div style={{ display: 'flex', gap: 8 }}>
                <button type="submit" style={{ flex: 1, height: 40, background: 'var(--foreground)', color: 'var(--background)', border: 'none', borderRadius: 10, fontSize: 13.5, fontWeight: 600, cursor: 'pointer' }}>Continue</button>
                <button type="button" onClick={() => setLoginOpen(false)} style={{ height: 40, padding: '0 14px', background: 'transparent', color: 'var(--muted)', border: '1px solid var(--border)', borderRadius: 10, fontSize: 13.5, cursor: 'pointer' }}>Cancel</button>
              </div>
            </form>
          </div>
        </div>
